@props(['players', 'showRelease' => false])

<div class="card-section">
    @forelse ($players as $player)
        <div class="row-hover flex items-center gap-3 px-4 py-2" style="border-bottom: 1px solid var(--line);">
            <a href="{{ route('manager.players.show', $player) }}" class="flex items-center gap-3" style="flex: 1 1 auto; min-width: 0; text-decoration: none;">
            ...
            @if ($showRelease && ! $player->is_manager_player)
                <form method="POST" action="{{ route('manager.team.players.release', $player) }}" onsubmit="return confirm('Release {{ $player->name }}? They become a free agent again — your budget spent on them is not refunded.');">
                    @csrf
                    <button type="submit" class="btn-ghost px-3 py-1.5 text-xs whitespace-nowrap" style="color: var(--live);">Release</button>
                </form>
            @endif
        </div>
    @empty
