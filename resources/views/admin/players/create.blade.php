...
@section('content')
    <div class="mb-8">
        <h1 class="text-4xl font-black text-white">Add New Player</h1>
    </div>
    ...
    <div class="cricket-card rounded-2xl p-8 max-w-2xl">
        <form action="{{ route('admin.players.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="name" class="w-full px-4 py-3 rounded-lg bg-slate-800 border border-slate-700 text-white placeholder-slate-500 focus:border-emerald-500 focus:outline-none transition" placeholder="e.g., Virat Kohli" value="{{ old('name') }}" required>
                @error('name') <p class="text-red-400 text-sm mt-1">{{ $message }}</p> @enderror
            </div>

            <div>
                <label class="block text-sm font-bold text-white mb-2">Team (Optional)</label>
            ...
            <div>
                ...
                <input type="number" name="base_value" class="w-full px-4 py-3 rounded-lg bg-slate-800 border border-slate-700 text-white placeholder-slate-500 focus:border-emerald-500 focus:outline-none transition" placeholder="500000" value="{{ old('base_value', 500000) }}" required>
